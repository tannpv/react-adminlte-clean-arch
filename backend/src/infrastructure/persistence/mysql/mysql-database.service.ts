...
export class MysqlDatabaseService implements OnModuleInit, OnModuleDestroy {
  ...
  private async seedDefaults(): Promise<void> {
    ...
    if (roleCount === 0) {
      ...
      this.logger.log(
        `Seeded default roles (Admin=${adminId}, User=${userId}, Translator=${translatorId}, Seller=${sellerId})`
      );
    } else {
  ...
  private async seedDefaultTranslations(): Promise<void> {
    ...
    if (englishId) {
      ...
      const englishTranslations = {
        ...
        "stores.descriptionPlaceholder": "Enter store description",
        "stores.logoUrl": "Logo URL",
        "stores.logoUrlPattern": "Please enter a valid URL",
        "stores.logoUrlPlaceholder": "https://example.com/logo.png",
        "stores.bannerUrl": "Banner URL",
        "stores.bannerUrlPattern": "Please enter a valid URL",
        "stores.bannerUrlPlaceholder": "https://example.com/banner.png",
        "stores.commissionRate": "Commission Rate (%)",
        "stores.commissionRateRequired": "Commission rate is required",
        "stores.commissionRateMin": "Commission rate must be at least 0",
        "stores.commissionRateMax": "Commission rate must be at most 100",
        "stores.owner": "Owner",
        "stores.status": "Status",
        "stores.commission": "Commission",
        "stores.createdAt": "Created At",
        "stores.status.pending": "Pending",
        "stores.status.approved": "Approved",
        "stores.status.suspended": "Suspended",
        "stores.status.rejected": "Rejected",
        "stores.approve": "Approve",
        "stores.reject": "Reject",
        "stores.suspend": "Suspend",
        "stores.reactivate": "Reactivate",
        "stores.totalStores": "Total Stores",
        "stores.pendingStores": "Pending Stores",
        "stores.approvedStores": "Approved Stores",
        ...
        "orders.order": "Order",
        "orders.ordersDescription": "Manage orders and track their status",
        "orders.noOrdersFound": "No orders found",
        "orders.noOrdersDescription": "No orders have been placed yet.",
        ...
        "orders.processingOrders": "Processing Orders",
