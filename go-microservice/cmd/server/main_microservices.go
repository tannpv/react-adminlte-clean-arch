...
func main() {
	...
	// Initialize router with JWT strategy
	router := setupRouterWithJWTStrategy(cfg, userHandler, productHandler)

	// Start server
	startServer(cfg, router, logger)
}

func setupRouterWithJWTStrategy(
	cfg *config.Config,
	...
	api := router.Group("/api/v1")
	{
		...
		// Protected routes with JWT strategy
		protected := api.Group("/")

		// Choose JWT middleware based on configuration
		authMiddleware := getJWTMiddleware(cfg)
...
// getJWTMiddleware returns the appropriate JWT middleware based on configuration
func getJWTMiddleware(cfg *config.Config) gin.HandlerFunc {
	jwtConfig := config.LoadJWTConfig()

	// Strategy 1: Centralized Auth Service
	if jwtConfig.IsCentralizedAuth() {
		log.Println("Using centralized authentication")
		authClient := middleware.NewAuthServiceClient(jwtConfig.AuthServiceURL)
		return middleware.CentralizedAuthMiddleware(authClient)
	}

	// Strategy 2: RSA Public/Private Key
	if jwtConfig.IsRSAAlgorithm() {
		log.Println("Using RSA public key authentication")
		publicKey, err := jwtConfig.LoadRSAPublicKey()
		if err != nil {
			log.Fatal("Failed to load RSA public key:", err)
		}
		return middleware.RSAAuthMiddleware(publicKey)
	}

	// Strategy 3: Shared Secret (default)
	log.Println("Using shared secret authentication")
	return middleware.SharedAuthMiddleware(jwtConfig.Secret)
}
