...
export function RolesPage({ getRolesUseCase, createRoleUseCase, updateRoleUseCase, deleteRoleUseCase }) {
  ...
  const { data: roles = [], isLoading: loading, isError, error } = useQuery({ queryKey: ['roles'], queryFn: () => getRolesUseCase.execute() })
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, payload }) => updateRoleUseCase.execute(id, payload),
    onSuccess: async () => { await qc.invalidateQueries({ queryKey: ['roles'] }) },
  })
  const deleteMutation = useMutation({
    mutationFn: (id) => deleteRoleUseCase.execute(id),
    onSuccess: async () => { await qc.invalidateQueries({ queryKey: ['roles'] }) },
  })
